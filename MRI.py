...
main = tkinter.Tk()
main.title("An Overview of Deep Learning in Medical Imaging Focusing on MRI") #designing main screen
...
def predict():
    ftp = ftplib.FTP("ftp.drivehq.com")  # Use regular FTP instead of FTP_TLS
    ftp.login("codebook.in@gmail.com", "Cloud@123")
    
    name = imagelist.get()
    with open('drivehq.jpg', 'wb') as file:
        ftp.retrbinary('RETR %s' % name, file.write)
    
    ftp.quit()  # Remember to close the FTP connection
...
# You can call the functions predict() and getImages() as needed.


font = ('times', 16, 'bold')
title = Label(main, text='An Overview of Deep Learning in Medical Imaging Focusing on MRI')
